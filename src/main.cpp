...
private:
    ...
    bool pickWater(double sx, double sy, glm::vec2& out) {
        ...
        if (poolShape == 1) { if (lim > 1.0f) return false; }
        else if (lim > 1.0f) return false;
    ...
    void createSwapchain() {
        ...
        if (caps.currentExtent.width != UINT32_MAX) swapExtent = caps.currentExtent;
            ...
            RECT cr; GetClientRect(hwnd, &cr); uint32_t w = (uint32_t)(cr.right - cr.left), h = (uint32_t)(cr.bottom - cr.top);
    ...
    void loadObject() {
        ...
        for (size_t i = 0; i < vc; ++i) {
            verts.push_back(P[i].x); verts.push_back(P[i].y); verts.push_back(P[i].z);
        ...
        if (!g.baseColorImage.empty()) {
            ...
            if (tex.ok) {
                destroyImage(objTex); objTex = make2DTexture((uint32_t)tex.w, (uint32_t)tex.h, tex.rgba);
    ...
    void createGeometry() {
        ...
        for (uint32_t i = 0; i < STK; ++i) for (uint32_t j = 0; j < SEC; ++j) {
            uint32_t a = i * (SEC + 1) + j, b = a + SEC + 1;
    ...
    void recordFrame(uint32_t imageIndex) {
        ...
        }
        {
            ...
            if (hasObject) {
                vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeObject);
                ...
                vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeSphere);
