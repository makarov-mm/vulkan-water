...
struct vec3 {
    float x = 0, y = 0, z = 0;
    vec3() = default;
    explicit vec3(const float s): x(s), y(s), z(s) { }
...
struct vec4 {
    ...
    vec4(const float x_, const float y_, const float z_, float w_): x(x_), y(y_), z(z_), w(w_) { }
...
inline vec3 operator*(const float s, const vec3 a)
{
	return { a.x * s, a.y * s, a.z * s };
}
